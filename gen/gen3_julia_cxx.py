...
def gen(srcfiles, preprocessor_definitions):
    ...
    jl_code = StringIO()
    for name, ns in sorted(namespaces.items(), key=lambda x: x[0]):  # sorted(): reproducible processing order
        ...
        imports = ''
        for namex in sorted(namespaces):  # sorted(): reproducible submodule include order
