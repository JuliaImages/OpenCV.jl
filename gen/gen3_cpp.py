...
struct SuperType<%s>
{
    ...
    # print(enums)
    for name, ns in sorted(namespaces.items(), key=lambda x: x[0]):  # sorted(): reproducible namespace body order
    ...
    default_values = sorted(set(default_values))  # sorted(): reproducible default-value method order
